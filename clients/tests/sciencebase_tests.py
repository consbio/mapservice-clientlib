...
class ScienceBaseTestCase(ResourceTestCase):
    def setUp(self):
        ...
        arcgis_service_url = (
            "https://www.sciencebase.gov/arcgis/rest/services/Catalog/service/MapServer"
        )

        self.arcgis_item_url = (
            "https://www.sciencebase.gov/catalog/item/arcgis/?format=json"
        )
        ...
        self.arcgis_service_layer_path = (
            self.sciencebase_directory / "arcgis-service-layer.json"
        )
        self.arcgis_service_layers_url = f"{arcgis_service_url}/layers?f=json"
        self.arcgis_service_layers_path = (
            self.sciencebase_directory / "arcgis-service-layers.json"
        )
        self.arcgis_service_legend_url = f"{arcgis_service_url}/legend/?f=json"
        self.arcgis_service_legend_path = (
            self.sciencebase_directory / "arcgis-service-legend.json"
        )
        ...
        self.deprecated_url = (
            "https://www.sciencebase.gov/directory/deprecatedLegacyObj/getObj?objId=val"
        )
        ...
        self.wms_service_url = (
            f"{wms_service_url}?version=1.3.0&service=wms&request=getcapabilities"
        )
        self.wms_service_path = self.sciencebase_directory / "wms-service.xml"
        self.wms_service_image_url = (
            f"{wms_service_url}?version=1.3.0&service=wms&request=getcapabilities"
        )
        ...
        self.empty_error_url = (
            "https://www.sciencebase.gov/catalog/item/empty/?format=json"
        )
        self.empty_error_path = self.sciencebase_directory / "empty-error.json"

        self.footprint_url = (
            "https://www.sciencebase.gov/catalog/item/footprint/?format=json"
        )
        ...
        self.message_error_url = (
            "https://www.sciencebase.gov/catalog/item/message/?format=json"
        )
        self.message_error_path = self.sciencebase_directory / "message-error.json"

        self.messages_error_url = (
            "https://www.sciencebase.gov/catalog/item/messages/?format=json"
        )
        self.messages_error_path = self.sciencebase_directory / "messages-error.json"

        self.missing_fields_url = (
            "https://www.sciencebase.gov/catalog/item/missing-fields/?format=json"
        )
        self.missing_fields_path = self.sciencebase_directory / "missing-fields.json"

        self.unpublished_url = (
            "https://www.sciencebase.gov/catalog/item/error/?format=json"
        )
    ...
    def mock_service_client(self, mock_request, service_type, token=None):
        ...
        if service_type == "error":
            self.mock_mapservice_request(
                mock_request.get, self.arcgis_item_url, self.arcgis_item_path, ok=False
            )
            self.mock_mapservice_request(
                mock_request.get, self.wms_item_url, self.wms_item_path, ok=False
            )
            self.mock_mapservice_request(
                mock_request.get, self.empty_error_url, self.empty_error_path, ok=False
            )
            self.mock_mapservice_request(
                mock_request.get, self.json_error_url, self.json_error_path, ok=False
            )
            self.mock_mapservice_request(
                mock_request.get,
                self.message_error_url,
                self.message_error_path,
                ok=False,
            )
            self.mock_mapservice_request(
                mock_request.get,
                self.messages_error_url,
                self.messages_error_path,
                ok=False,
            )
            self.mock_mapservice_request(
                mock_request.get, self.missing_fields_url, self.missing_fields_path
            )
            self.mock_mapservice_request(
                mock_request.get, self.footprint_url, self.footprint_path
            )
            self.mock_mapservice_request(
                mock_request.get, self.unpublished_url, self.unpublished_path
            )

        elif service_type == "session":
            self.mock_mapservice_request(
                mock_request.get, self.arcgis_item_url, self.arcgis_item_path
            )
            self.mock_mapservice_request(
                mock_request.get, self.deprecated_url, self.deprecated_path
            )
            self.mock_mapservice_request(
                mock_request.get, self.wms_item_url, self.wms_item_path
            )
            ...
            for url, path in self.arcgis_service_data:
                self.mock_mapservice_request(
                    mock_request.get, url_format.format(url=url, token=token), path
                )
    ...
    def assert_sciencebase_session(self, client, username=None, token=None):
        ...
        self.assertTrue(
            session._session.headers.get("Accept"), SCIENCE_BASE_CONTENT_TYPES
        )
    ...
    @requests_mock.Mocker()
    @mock.patch(
        "clients.sciencebase.SbSession.login", new_callable=mock_sbsession_login
    )
    def test_valid_sciencebase_sessions(self, mock_request, mock_login):
        ...
        session_kwargs = {"token": "usgs_session.id", "username": "usgs_session.uid"}

        session = None
        client = ScienceBaseResource.get(
            self.arcgis_item_url, session=session, **session_kwargs
        )
        ...
        session = ScienceBaseSession(
            josso_session_id="usgs_session.id", username="usgs_session.uid"
        )
        client = ScienceBaseResource.get(
            self.arcgis_item_url, session=session, **session_kwargs
        )
    ...
    def test_valid_arcgis_sciencebase_request(self, mock_request, mock_server_admin):
        ...
        mock_server_admin.return_value = mock.Mock(
            token="arcgis_token", generate_token=mock.Mock()
        )
        token, username = "usgs_session.id", "usgs_session.uid"
        arcgis_credentials = {"username": "arcgis_user", "password": "arcgis_pass"}
        ...
        self.assertEqual(
            client.josso_credentials, {"josso": token, "username": username}
        )
        self.assertEqual(
            client.arcgis_credentials,
            {"token": "arcgis_token", "username": "arcgis_user"},
        )
        ...
        self.assertEqual(
            service_client.arcgis_credentials,
            {"token": "arcgis_token", "username": "arcgis_user"},
        )
        ...
        self.assertEqual(
            client.description,
            "With reference to white tailed jack rabbit found in Alaska",
        )
        ...
        self.assert_object_field(
            ...
            [
                ...
                },
                {
                    ...
                    "contact_type": "person",
                    "organization": {
                        "display_text": "Southern Rockies Landscape Conservation Cooperative"
                    },
                },
                {
        ...
        self.assertEqual(
            ...
            [
                "USER:bcward@consbio.org",
                "USER:dani.harvey@consbio.org",
                "USER:databasinadmin@consbio.org",
            ],
        )
    ...
    def test_valid_wms_sciencebase_request(self, mock_request, mock_login):
        ...
        client = ScienceBaseResource.get(
            self.wms_item_url, lazy=False, username=username, password="usgs_pass"
        )
        ...
        self.assertEqual(
            service_client.wms_credentials, {"josso": token, "token_id": "josso"}
        )
        ...
        self.assertEqual(client.service_version, service_client.version)
        self.assertEqual(
            client.josso_credentials,
            {"josso": token, "username": username, "token_id": "josso"},
        )
        ...
        self.assertEqual(
            client.purpose,
            "This dataset is being used as part of the Conservation Blueprint 1.0.",
        )
        self.assertEqual(client.use_constraints, None)
        ...
        self.assertEqual(
            client.properties,
            {"wms_layer_name": "SouthwesternWillowFlycatcher_FocalArea"},
        )
        self.assertEqual(client.system_types, ["Downloadable", "Mappable"])
    ...
    def test_valid_wms_sciencebase_image_request(self, mock_request):
        ...
        self.assert_get_image(
            client, layer_ids=["AKCAN_mastersample_50km"], style_ids=["highlight"]
        )
...
ARCGIS_SETTINGS = {
    "permissions": {
        "read": {"user": ["bcward@consbio.org", "dani.harvey@consbio.org"]},
        "write": None,
    },
    "private": True,
