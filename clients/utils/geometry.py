...
class Extent(object):
    ...
    def __init__(self, extent=None, spatial_reference=None):
        ...
        if extent is None:
        ...
        elif hasattr(extent, "xmin"):  # Instance of self or similar object
            try:
                ...
                raise BadExtent("Invalid extent: must be a compatible object", extent=extent, underlying=ex)
            ...
            try:
                ...
                raise BadExtent("Invalid extent: missing required keys", extent=extent, underlying=ex)
    ...
    def as_list(self):
        return [getattr(self, key) for key in EXTENT_KEYS]
    ...
    def fit_image_dimensions_to_extent(self, width, height, target_resolution=None):
        ...
        if img_aspect_ratio > extent_aspect_ratio:
            # img is taller than extent
            diffExtentUnits = ((img_aspect_ratio * x_diff) - y_diff) / 2.0
            offsetPixels = int(round(diffExtentUnits / resolution, 0))
            height = height - 2 * offsetPixels
        elif img_aspect_ratio < extent_aspect_ratio:
            # img is wider than extent
            diffExtentUnits = ((y_diff / img_aspect_ratio) - x_diff) / 2.0
            offsetPixels = int(round(diffExtentUnits / resolution, 0))
    ...
    def project_to_web_mercator(self):
        ...
        if source_srs == "EPSG:4326":
            if self.ymin <= -90:
                self.ymin = -85.0511  # Per open layers convention
            if self.ymax >= 90:
                self.ymax = 85.0511

        new_extent.xmin, new_extent.ymin, new_extent.xmax, new_extent.ymax = self._get_projected_extent(
    ...
    def _get_projected_extent(self, source_srs, target_srs, edge_points=9):
        """
        ...
        :return: the outer bounds of the projected coords.
        """
        ...
        if any(not is_number(coord) for coord in projected_values):
            ...
            raise ValueError(f'Invalid projection coordinates for "{source_srs}": {coords}')
    ...
    def from_sql_box(cls, box, spatial_reference):
        ...
        if not match:
            raise ValueError(f"Invalid SQL Box: {box}")
...
class TileLevels(object):
    ...
    def get_nearest_tile_level_and_resolution(self, target_resolution, allow_lower_resolution=False):
        ...
        if allow_lower_resolution:
            diffs = [fabs(target_resolution - resolution) for resolution in self.resolutions]
            zoom_level = diffs.index(min(diffs))
        else:
            sorted_resolutions = [round(resolution, 5) for resolution in self.resolutions]
