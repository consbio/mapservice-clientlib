...
from .query.arcgis import (
    FEATURE_LAYER_QUERY,
    FEATURE_LAYER_TIME_QUERY,
...
WEB_MERCATOR_SRS = "+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +units=m +k=1.0 +nadgrids=@null +no_defs"
...
class ArcGISSecureResource(ClientResource):
    ...
    def _get(self, url, username=None, password=None, token=None, **kwargs):
        ...
        self.service_id = (
            service_pattern_match.group() if service_pattern_match else None
        )
        ...
        if token:
            ...
            server_admin = ArcGISSecureResource.generate_token(
                self._url, username, password
            )
            if server_admin.token:
                self._params["token"] = server_admin.token

        self.arcgis_credentials["token"] = self._token = self._params.get(
            "token", token
        )
...
class ArcGISServerResource(ArcGISResource, ArcGISSecureResource):
    ...
    def populate_field_values(self, data):
        ...
        self.time_enabled = (
            self.time_info is not None
            and len(self.get_time_info_prop("time_extent", [])) == 2
            and all(
                self.get_time_info_prop(prop)
                for prop in ("default_interval", "default_units")
            )
        )
...
class ArcGISLayerResource(ArcGISResource, ArcGISSecureResource):
    ...
    ownership_based_access_control_for_features = ObjectField(
        class_name="FeatureAccessControl", required=False
    )
    ...
    def populate_field_values(self, data):
        ...
        self.time_enabled = (
            self.time_info is not None
            and len(self.get_time_info_prop("time_extent", [])) == 2
            and all(self.get_time_info_prop(prop) for prop in ("interval", "units"))
        )
...
class ArcGISTiledImageResource(ArcGISServerResource):
        ...
        try:
            if self.tile_info is not None:
                ...
                if extent.has_negative_extent():
                    ...
                    negative_image = self._get_tiled_image(
                        negative_extent, width, height
                    )
    ...
    def _get_tiled_image(self, extent, width, height):
        ...
        zoom_level, resolution = tile_levels.get_nearest_tile_level_and_resolution(
            target_resolution
        )

        try:
            if (
                math.fabs(resolution - target_resolution) > 0.0001
            ):  # Nearest tile resolution is not close enough!
                outside_range = (
                    self.tile_info.lods[-1].resolution - target_resolution > 0.0001
                    or target_resolution - self.tile_info.lods[0].resolution > 0.0001
                )
                if outside_range:
                    ...
                    error = "\n\t".join(
                        (
                            ...
                            "Target resolution:{:.5f}, calculated resolution:{:.5f}".format(
            ...
            available_first_row = int(
                math.floor(
                    (tile_origin.y - self.full_extent.ymax) / tile_height_in_map_units
                )
            )
            available_last_row = int(
                math.floor(
                    (tile_origin.y - self.full_extent.ymin) / tile_height_in_map_units
                )
            )
            available_first_col = int(
                math.floor(
                    (self.full_extent.xmin - tile_origin.x) / tile_width_in_map_units
                )
            )
            available_last_col = int(
                math.floor(
                    (self.full_extent.xmax - tile_origin.x) / tile_width_in_map_units
                )
            )

            offset_x_in_pixels = 0  # Refers to how far right and up the extent"s origin is compared to the origin tile
            shift_x = (
                0  # Refers to how far off it is from the boundary of the world extent
            )
            ...
            first_tile_col = max(
                int(math.floor(offset_x_in_pixels / tile_width_in_pixels)),
                available_first_col,
            )
            first_tile_row = max(
                int(math.floor(offset_y_in_pixels / tile_height_in_pixels)),
                available_first_row,
            )

            offset_from_first_x = offset_x_in_pixels - (
                first_tile_col * tile_width_in_pixels
            )
            offset_from_first_y = offset_y_in_pixels - (
                first_tile_row * tile_height_in_pixels
            )
            ...
            number_of_columns = (
                int(
                    math.ceil(
                        ((extent.xmax - extent.xmin) - shift_x)
                        / tile_width_in_map_units
                    )
                )
                + 1
            )
            number_of_rows = (
                int(
                    math.ceil(
                        ((extent.ymax - extent.ymin) - shift_y)
                        / tile_height_in_map_units
                    )
                )
                + 1
            )
            ...
            base_image = Image.new(
                "RGBA",
                (
                    number_of_columns * tile_width_in_pixels,
                    number_of_rows * tile_height_in_pixels,
                ),
            )
            ...
            for row_index, row in enumerate(range(first_tile_row, last_tile_row + 1)):
                for column_index, column in enumerate(
                    range(first_tile_col, last_tile_col + 1)
                ):
                    tile_threads.append(
                        Thread(
                            target=self._render_single_tile,
                            args=(
                                zoom_level,
                                row,
                                column,
            ...
            cropped_image = base_image.crop(
                (
                    offset_from_first_x,
                    offset_from_first_y,
                    width + offset_from_first_x,
                    height + offset_from_first_y,
                )
            )
            ...
            if shift_x != 0 or shift_y != 0:
                ...
                shifted_image.paste(
                    cropped_image,
                    (
                        int(math.ceil(shift_x / resolution)),
                        int(math.ceil(shift_y / resolution)),
                    ),
                )
    ...
    def validate_tile_scheme(self):
        ...
        if (
            getattr(self.tile_info.spatial_reference, "wkid", None)
            not in WEB_MERCATOR_WKIDs
        ):
            message = "ArcGIS tiled service missing required wkid for mercator"
        elif self.tile_info.rows != self.tile_info.cols or self.tile_info.rows not in (
            256,
            512,
        ):
            message = "ArcGIS tiled service missing required dimensions"
        elif not ARCGIS_TILEINFO_RESOLUTIONS.get_matching_resolutions(
            [lod.resolution for lod in self.tile_info.lods]
        ):
            message = "ArcGIS tiled service LODs do not match at least one of the base map LODs"

        if message is not None:
            raise BadTileScheme(
                message=message, tile_info=self.tile_info, url=self._url
            )
...
class MapLayerResource(ArcGISLayerResource):
    ...
    sub_layers = ObjectField(
        class_name="ChildLayer", required=False, aliases={"name": "title"}
    )
    legend = ObjectField(
...
class MapServerResource(ArcGISTiledImageResource):
    ...
    version = NumberField(
        name="currentVersion", required=False
    )  # Overridden not to require
    ...
    def populate_field_values(self, data):
        ...
        self.layers = MapLayerResource.bulk_get(
            ...
            bulk_key="layers",
            bulk_defaults={"currentVersion": self.version},
            **self.arcgis_credentials,
        )
        if not self.layers:
            raise NoLayers(
                "The ArcGIS map service does not have any layers", url=self._url
            )
        ...
        for layer in (
            l for l in self.layers if "raster" in l.type.lower() and len(l.legend) >= 3
        ):
            max_colors = max(
                [count_colors(base64_to_image(l.image_base64)) for l in layer.legend]
            )
            ...
            if max_colors and max_colors > 3 and len(layer.legend) == 3:
                ...
                layer.legend[1].image_base64 = image_to_base64(
                    stack_images_vertically(images)
                )
    ...
    def get_image(
        ...
        **kwargs,
        ...
        if custom_renderers is not None:
            ...
            image_params["dynamicLayers"] = self._generate_dynamic_layers(
                custom_renderers, layer_defs, layers
            )
            image_params.pop(
                "layerdefs"
            )  # Dynamic layers take over for layerdefs: this saves URL space

        return self.generate_image_from_query(
            extent, width, height, "export", params=image_params
        )
    ...
    def _generate_dynamic_layers(self, custom_renderers, layer_defs, layers):
        ...
        if "hide" not in layers:
            layer_map = {
                visible_id: layer_list[visible_id] for visible_id in layer_list
            }
        else:
        ...
        for layer_id in layer_map:
            ...
            if layer_id in custom_renderers and custom_renderers[layer_id]:
                ...
                renderer = to_renderer(
                    custom_renderers[layer_id], from_camel=False
                ).get_data()
...
TIME_SUMMARY_RENDERER = to_renderer(
    setdefaults(
        {},
        {
            "description": "",
...
class FeatureLayerResource(ArcGISLayerResource):
    ...
    def get_image(
        ...
        **kwargs,
        ...
        while True:
            object_ids_to_get = object_ids[
                starting_record : starting_record + max_features
            ]
            ...
            id_where_clause = "{object_field} IN {formatted_id_list}".format(
                ...
                formatted_id_list=json.dumps(object_ids_to_get)
                .replace("[", "(")
                .replace("]", ")"),
            )
            query_results = self.query(
                where=id_where_clause, out_sr=3857, out_fields="*", **query_kwargs
            )
            ...
            sub_image = self.generate_sub_image(
                extent, width, height, renderer, query_results
            )
            full_image = overlay_images(
                [full_image, sub_image], background_color=transparent_background
            )
    ...
    def get_time_image(self, extent, width, height, **kwargs):
        query_kwargs = {
            k: v for k, v in kwargs.items() if k in FEATURE_LAYER_TIME_PARAMS
        }
        if any(k not in query_kwargs for k in kwargs):
            extras = ", ".join(k for k in kwargs if k not in query_kwargs)
            logger.warning(f"Ignoring {self.client_name} query fields: {extras}")
        ...
        return self.generate_sub_image(
            ...
            width,
            height,
            TIME_SUMMARY_RENDERER,
            self.time_query(**query_kwargs),
        )
...
class FeatureServerResource(ArcGISServerResource):
    ...
    layers = ToManyField(
        FeatureLayerResource, "partial", id_field="id", relative_path="{id}"
    )
    ...
    def populate_field_values(self, data):
        ...
        if not self.layers:
            raise NoLayers(
                "The ArcGIS feature service does not have any layers", url=self._url
            )

    def get_image(
        self, extent, width, height, custom_renderers=None, layer_defs=None, **kwargs
    ):
        ...
        for layer in self.layers:
            final_image.paste(
                layer.get_image(
                    extent, width, height, custom_renderers, layer_defs, **kwargs
                )
            )
...
class ImageServerResource(ArcGISTiledImageResource):
    ...
    object_id_field = TextField(required=False)
    ownership_based_access_control_for_rasters = ObjectField(
        class_name="AccessControlForRasters", required=False
    )
    ...
    def get_image(self, extent, width, height, **kwargs):
        ...
        image_params = {
            ...
            "time": ",".join(
                str(i) for i in self.get_time_info_prop("time_extent", [])
            ),
        }
        image_params.update(kwargs)

        return self.generate_image_from_query(
            extent, width, height, "exportImage", params=image_params
        )
